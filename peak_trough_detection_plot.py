...
def convert_to_nyc_timezone(timestamp_str):
    ...
    try:
        ...
        try:
            return date_parser.parse(timestamp_str) if isinstance(timestamp_str, str) else timestamp_str
        except:
...
def process_bars_for_peaks(symbol, bars, window_len=11, lookahead=1):
    ...
    try:
        ...
        logger.info("Processed %d bars for %s: %d peaks, %d troughs", len(bars), symbol, len(peaks_data), len(troughs_data))
...
def print_latest_signals_table(all_results):
    ...
    header_items = ['Symbol', 'Signal', 'Ago', 'Signal $', 'Current $', 'Change', 'Change%', 'Time (NYC/EDT)']
    ...
    for results in all_results:
        ...
        if latest_signal:
            ...
            cols = [
                ...
                "{}{}".format(change_indicator, "{:.4f}".format(abs(price_change))).rjust(col_widths[5]),
...
def plot_single_symbol(results, save_plot=False, output_dir="."):
    """Create a professional plot for a single symbol"""
    if not results:
    ...
    ax.plot(timestamps, original_prices, 
           ...
           label='Original Close Prices', zorder=1)
    
    ax.plot(timestamps, filtered_prices, 
           color='#A23B72', linewidth=2.5, 
           label='Filtered (Hanning w={})'.format(results["filter_params"]["window_len"]), zorder=2)
    
    # Plot peaks at original prices with actual price annotations
    if peaks:
        peak_times = [convert_to_nyc_timezone(peak['timestamp']) if isinstance(timestamps[0], datetime) 
                     else peak['index'] for peak in peaks]
        peak_original_prices = [peak['original_price'] for peak in peaks]
        
        ax.scatter(peak_times, peak_original_prices, 
                  color='#F18F01', s=80, marker='^', 
                  label='Peaks ({})'.format(len(peaks)), zorder=4, edgecolors='black', linewidth=1)
        
        # Add peak annotations with actual prices
        for i, (time, price) in enumerate(zip(peak_times, peak_original_prices)):
            ax.annotate('P{}: ${:.4f}'.format(i+1, price), (time, price), 
    ...
    if troughs:
        trough_times = [convert_to_nyc_timezone(trough['timestamp']) if isinstance(timestamps[0], datetime) 
                       else trough['index'] for trough in troughs]
        trough_original_prices = [trough['original_price'] for trough in troughs]
        
        ax.scatter(trough_times, trough_original_prices, 
                  color='#C73E1D', s=80, marker='v', 
                  label='Troughs ({})'.format(len(troughs)), zorder=4, edgecolors='black', linewidth=1)
        
        # Add trough annotations with actual prices
        for i, (time, price) in enumerate(zip(trough_times, trough_original_prices)):
            ax.annotate('T{}: ${:.4f}'.format(i+1, price), (time, price), 
    ...
    title = ('{} - Peak & Trough Detection\n'
            ...
            'Lookahead: {} | Peaks: {} | Troughs: {}').format(
            symbol, total_bars, window_len, lookahead, len(peaks), len(troughs))
    ...
    # Add stats text box with actual price values
    ...
    stats_text = ('Price Range: ${:.4f} - ${:.4f}\n'
                 'Filter Smoothing: {:.1f}%\n'
                 'Peak/Trough Ratio: {}/{}').format(
                 price_min, price_max, noise_reduction, len(peaks), len(troughs))
    ...
    if save_plot:
        ...
        filename = os.path.join(output_dir, '{}_peak_detection_{}.png'.format(symbol, timestamp))
...
def plot_combined_subplots(all_results, save_plot=False, output_dir="."):
    ...
    for idx, results in enumerate(all_results):
        ...
        # Plot lines
        ax.plot(timestamps, original_prices, color='#2E86AB', linewidth=1.5, alpha=0.7, label='Original')
        ...
        if peaks:
            ...
            peak_original_prices = [peak['original_price'] for peak in peaks]
            ax.scatter(peak_times, peak_original_prices, color='#F18F01', s=60, marker='^', 
                      label='Peaks ({})'.format(len(peaks)), zorder=4, edgecolors='black')
            
            # Add price annotations for peaks
            for i, (time, price) in enumerate(zip(peak_times, peak_original_prices)):
                ax.annotate('${:.4f}'.format(price), (time, price), 
                           xytext=(0, 10), textcoords='offset points',
                           fontsize=8, ha='center', color='#F18F01')
        
        if troughs:
            trough_times = [convert_to_nyc_timezone(trough['timestamp']) if isinstance(timestamps[0], datetime) 
                           else trough['index'] for trough in troughs]
            trough_original_prices = [trough['original_price'] for trough in troughs]
            ax.scatter(trough_times, trough_original_prices, color='#C73E1D', s=60, marker='v', 
                      label='Troughs ({})'.format(len(troughs)), zorder=4, edgecolors='black')
        ...
        # Styling
        symbol = results['symbol']
        ax.set_title('{} (P:{} T:{})'.format(symbol, len(peaks), len(troughs)), fontweight='bold')
        ax.set_ylabel('Price ($)')
    ...
    fig.suptitle('Multi-Symbol Peak Detection (Hanning w={}, Lookahead={})'.format(window_len, lookahead), 
    ...
    if save_plot:
        ...
        filename = os.path.join(output_dir, 'multi_symbol_{}_{}.png'.format(symbols_str, timestamp))
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
...
def plot_overlay(all_results, save_plot=False, output_dir="."):
    ...
    for idx, results in enumerate(all_results):
        ...
        # Plot lines
        ax.plot(timestamps, normalized_prices, color=color, linewidth=1.5, alpha=0.7, 
               label='{} Original'.format(symbol))
        ax.plot(timestamps, normalized_filtered, color=color, linewidth=2.5, 
               label='{} Filtered'.format(symbol), linestyle='--')
        ...
        if peaks:
            ...
            peak_normalized = [(peak['original_price'] / original_prices[0] - 1) * 100 for peak in peaks]
            ax.scatter(peak_times, peak_normalized, color=color, s=80, marker='^', 
                      edgecolors='black', zorder=4)
            
            # Add actual price annotations
            for i, (time, norm_price, actual_price) in enumerate(zip(peak_times, peak_normalized, [p['original_price'] for p in peaks])):
                ax.annotate('${:.4f}'.format(actual_price), (time, norm_price), 
        ...
        if troughs:
            ...
            trough_normalized = [(trough['original_price'] / original_prices[0] - 1) * 100 for trough in troughs]
            ax.scatter(trough_times, trough_normalized, color=color, s=80, marker='v', 
                      edgecolors='black', zorder=4)
            
            # Add actual price annotations
            for i, (time, norm_price, actual_price) in enumerate(zip(trough_times, trough_normalized, [t['original_price'] for t in troughs])):
                ax.annotate('${:.4f}'.format(actual_price), (time, norm_price), 
                           xytext=(5, -15), textcoords='offset points',
                           fontsize=8, color=color)
    ...
    ax.set_title('Multi-Symbol Overlay (Normalized Returns with Actual Prices)', fontsize=14, fontweight='bold')
    ...
    if save_plot:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        symbols_str = '_'.join([r['symbol'] for r in all_results])
        filename = os.path.join(output_dir, 'overlay_{}_{}.png'.format(symbols_str, timestamp))
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
...
def print_summary(results):
    ...
    print("   • Filter parameters: window_len={}, lookahead={}".format(
        results['filter_params']['window_len'], results['filter_params']['lookahead']))
    print("   • Price range: ${:.4f} - ${:.4f}".format(
    ...
    if results['peaks'] or results['troughs']:
        if results['peaks']:
            ...
            for i, peak in enumerate(results['peaks']):
                print("     P{}: Index {}, ${:.4f} (filtered: ${:.4f})".format(
                    i+1, peak['index'], peak['original_price'], peak['filtered_price']))
        
        if results['troughs']:
            print("   TROUGHS (at original close prices):")
            for i, trough in enumerate(results['troughs']):
                print("     T{}: Index {}, ${:.4f} (filtered: ${:.4f})".format(
                    i+1, trough['index'], trough['original_price'], trough['filtered_price']))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="1Min",
        choices=["1Min", "2Min", "5Min", "15Min", "30Min", "1Hour", "2Hour", "4Hour", "1Day"],
        help="Bar timeframe"
    )
    ...
    try:
        ...
        if not args.no_plot:
            if args.plot_mode == "single" or args.plot_mode == "all":
                ...
                for results in all_results:
                    plot_single_symbol(results, save_plot=args.save_plots, output_dir=args.output_dir)
            
            if (args.plot_mode == "combined" or args.plot_mode == "all") and len(all_results) > 1:
                logger.info("Creating combined subplot...")
                plot_combined_subplots(all_results, save_plot=args.save_plots, output_dir=args.output_dir)
        ...
        print("\n🎯 OVERALL SUMMARY:")
        print("   • Symbols processed: {}/{}".format(len(all_results), len(test_symbols)))
