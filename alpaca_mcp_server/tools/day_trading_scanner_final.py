"""Final corrected day trading scanner with proper data consolidation."""
...
async def scan_day_trading_opportunities(
    ...
    try:
        ...
        for section in sections[1:]:  # Skip header
            if "- Complete Market Data" not in section:
                continue
            ...
            # Initialize all metrics for this symbol
            ...
            # Parse ALL lines for this symbol, collecting data from all sections
            for line in lines:
                line = line.strip()
                ...
                # Volume lines - distinguish between minute and daily volume
                elif line.startswith("• Volume:"):
                    vol_text = line.replace("• Volume: ", "").replace(",", "")
                    try:
                        volume_value = int(float(vol_text))
                        # Heuristic: minute volumes are typically < 1M, daily volumes > 1M
                        if volume_value < 1000000:  # Less than 1M = minute volume
                            minute_volume = volume_value
                        # If > 1M, it's likely daily volume (we'll skip it)
                    except Exception:
                        pass
            ...
            print(
                f"CONSOLIDATED {symbol}: Price=${current_price:.2f}, MinuteVol={minute_volume:,}, Trades={trades_per_minute}, Change={percent_change:.2f}%"
            )
...
• Reduce min_percent_change requirement (try 0.5-2%)
• Scan during more active market hours (9:30-11:30 AM, 2:30-4:00 PM ET)
