            ...
            if current_symbol and symbol_data:
                ...
                if line.startswith('• Price: $'):
                    ...
                    if symbol_data['current_price'] > 0 and symbol_data['prev_close'] > 0:
                        # Calculate metrics
                        percent_change = ((symbol_data['current_price'] - symbol_data['prev_close']) / 
                                        symbol_data['prev_close'] * 100)
                        
                        # After-hours specific calculations
                        ah_range = symbol_data['high'] - symbol_data['low'] if symbol_data['high'] > 0 else 0
                        ah_range_pct = (ah_range / symbol_data['prev_close'] * 100) if symbol_data['prev_close'] > 0 else 0
                        
                        # Liquidity assessment
                        spread_pct = (symbol_data['spread'] / symbol_data['current_price'] * 100) if symbol_data['current_price'] > 0 else 0
                        
                        # Apply filters
                        if (abs(percent_change) >= min_percent_change and 
                            ...
                            symbol_data.update({
                                ...
                                'momentum_score': abs(percent_change) * (symbol_data['volume'] / 1000000),
                                'liquidity_score': max(0, 100 - (spread_pct * 10))  # Lower spread = higher score
                            })
...
async def get_enhanced_streaming_analytics(
    ...
    try:
        ...
        try:
            trades_data = await get_stock_stream_data(symbol, "trades", recent_seconds=analysis_minutes*60)
            quotes_data = await get_stock_stream_data(symbol, "quotes", recent_seconds=analysis_minutes*60) if include_orderbook else ""
        except Exception:
