...
class StockAnalyzer:
    ...
    def analyze_momentum_exhaustion(self, snapshot):
        try:
            ...
            price_position = (
                ...
                if (daily_high - daily_low) > 0 else 0.5
            )
    ...
    def generate_signals(self, snapshot, breakout_data, momentum_data, volume_data, velocity_data, volatility_data):
        signals = []
        signal_strength = 0
        ...
        try:
            ...
            daily_high = float(snapshot.daily_bar.high or 0)
            daily_low = float(snapshot.daily_bar.low or 0)
            avg_volume = np.mean(volume_data['vol_ratio']) if isinstance(volume_data['vol_ratio'], list) else volume_data['vol_ratio']
...
def run(args):
    ...
    if symbol_list:
        try:
            ...
            universe = []
            print(f"No symbols to process.")
            return
    else:
        print(f"No symbol list provided.")
        return
    ...
    historical_data = asyncio.run(fetch_historical_data(universe, timeframe=timeframe, trading_days=trading_days))
    ...
    for symbol in universe:
        try:
            ...
            volume_data = analyzer.analyze_volume_pattern(snapshot, historical_volumes, historical_trades)
            ...
            signals, signal_strength = analyzer.generate_signals(
                snapshot, breakout_data, momentum_data, volume_data, velocity_data, volatility_data
            )
    ...
    for result in analysis_results:
        ...
        print(f"{result['symbol']:<6} {result['price']:8.3f} {result['percent_change']:8.2f} "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--list", type=str, default="combined.lis", help="Symbol list (default: combined.lis)")
    parser.add_argument("--min-price", type=float, default=0.0, help="Minimum price filter (default=0.0)")
    parser.add_argument("--min-volume", type=int, default=0, help="Minimum volume filter (default=0)")
    parser.add_argument("--sort", type=str, default="percent_change",
                        help="Sort column (percent_change, signal_strength, volume_ratio, etc.)")
    parser.add_argument("--timeframe", type=str, default="1Day", help="Timeframe for historical data (default: 1Day)")
    parser.add_argument("--trading-days", type=int, default=15, help="Number of trading days for baseline computations (default: 15)")
